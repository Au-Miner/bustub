...
class TrieNode {
 public:
  ...
  TrieNode(TrieNode &&other_trie_node) noexcept {
    LOG_DEBUG("出现在了TrieNode(TrieNode &&other_trie_node)中！");
  ...
  std::unique_ptr<TrieNode> *InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) {
    if (children_.count(key_char) || key_char != child->key_char_)
      return nullptr;
...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    {
      if (key.empty())
        return false;
      latch_.WLock();
      if (!root_->HasChild(key[0])) {
        root_->InsertChildNode(key[0], std::make_unique<TrieNode>(key[0]));
      }
      bool res = HandleInsert(key, value, root_->GetChildNode(key[0]), 0);
      latch_.WUnlock();
      return res;
    }
  }
  ...
  template <typename T>
  bool HandleInsert(const std::string &key, T value, std::unique_ptr<TrieNode> *rt, int pos) {
    if (pos == int(key.length()) - 1) {
      if (rt->get()->IsEndNode())
        return false;
      *rt = std::make_unique<TrieNodeWithValue<T>>(std::move(**rt), value);
      return true;
    }
    if (!rt->get()->HasChild(key[pos + 1])) {
      std::unique_ptr<TrieNode> newRt = std::make_unique<TrieNode>(key[pos + 1]);
      rt->get()->InsertChildNode(key[pos + 1], std::move(newRt));
    }
    return HandleInsert(key, value, rt->get()->GetChildNode(key[pos + 1]), pos + 1);
  }
  ...
  bool Remove(const std::string &key) {
    int success = false;
    latch_.WLock();
    HandleRemove(key, root_->GetChildNode(key[0]), 0, success);
    latch_.WUnlock();
    return success;
  }

  auto HandleRemove(const std::string &key, std::unique_ptr<TrieNode> *rt, int pos, int &success) -> bool {
    if (rt == nullptr)
      return false;
    if (pos == int(key.length()) - 1) {
      if (rt->get()->IsEndNode()) {
      ...
      return !rt->get()->HasChildren();
    }
    if (!HandleRemove(key, rt->get()->GetChildNode(key[pos + 1]), pos + 1, success))
      return false;
    else {
      rt->get()->RemoveChildNode(key[pos + 1]);
      return !rt->get()->HasChildren() && !rt->get()->IsEndNode();
    }
  }
  ...
  T  GetValue(const std::string &key, bool *success) {
    *success = false;
    if (key.empty())
      return T();
    latch_.WLock();
    T value = HandleGetValue<T>(key, root_->GetChildNode(key[0]), 0, success);
    latch_.WUnlock();
    return value;
  }

  template <typename T>
  auto HandleGetValue(const std::string &key, std::unique_ptr<TrieNode> *rt, size_t pos, bool *success) -> T {
    if (rt == nullptr)
      return T();
    if (pos == key.length() - 1) {
      if (rt->get()->IsEndNode()) {
        auto endPoint = dynamic_cast<TrieNodeWithValue<T>*>(rt->get());
        if (endPoint != nullptr) {
          *success = true;
    ...
    return HandleGetValue<T>(key, rt->get()->GetChildNode(key[pos + 1]), pos + 1, success);
  }


  template<typename T, typename U>
  bool _isType(const U& value) {
