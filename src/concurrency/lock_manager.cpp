...
namespace bustub {
    /*
     * 可重复读：
     *      Growing：    所有LockMode都可加锁
     *      Shrinking：  所有LockMode都不可加锁
     * 读取已提交：
     *      Growing：    所有LockMode都可加锁
     *      Shrinking：  只有S，IS可加锁
     * 读取未提交：
    ...
    auto LockManager::LockTable(Transaction *txn, LockMode lock_mode, const table_oid_t &oid) -> bool {
        ...
        // 判断LockRequestQueue中是否存在原txnId，若是则需要判断能否满足upgrade状态，如果可以需要删掉原来的txn
        for (auto request : lock_request_queue->request_queue_) {  // NOLINT
            if (request->txn_id_ == txn->GetTransactionId()) {
                ...
                // 更新此时LockRequestQueue的upgrading，upgrading代表当前正在进行upgrade的txnId
                // 因为不允许多个事务在同一资源上同时尝试锁升级，目的是为了降低实现复杂度=。=
                lock_request_queue->upgrading_ = txn->GetTransactionId();

                std::unique_lock<std::mutex> lock(lock_request_queue->latch_, std::adopt_lock);
                // 判断前面已经获取lock的txn的锁模式是否和当前锁模式冲突，如果冲突则使用条件变量等待
        ...
        // 前面是判断是否有相同txn需要upgrade，下面是单独新来的txn
    ...
    auto LockManager::UnlockTable(Transaction *txn, const table_oid_t &oid) -> bool {
        ...
        for (auto lock_request : lock_request_queue->request_queue_) {  // NOLINT
            if (lock_request->txn_id_ == txn->GetTransactionId() && lock_request->granted_) {
                ...
                // 在lock_request_queue中删掉具体的txn，并通知其他所有有关该table的条件变量cv_
                lock_request_queue->cv_.notify_all();
                lock_request_queue->latch_.unlock();
    ...
    auto LockManager::LockRow(Transaction *txn, LockMode lock_mode, const table_oid_t &oid, const RID &rid) -> bool {
        ...
        // 和LockTable一样判断有没有相同txn要upgrade
        for (auto request : lock_request_queue->request_queue_) {  // NOLINT
            if (request->txn_id_ == txn->GetTransactionId()) {
                if (request->lock_mode_ == lock_mode) {
                    lock_request_queue->latch_.unlock();
    ...
    auto LockManager::UnlockRow(Transaction *txn, const table_oid_t &oid, const RID &rid) -> bool {
        ...
        // 和unlockRow一致，先获取LockRequestQueue
        if (row_lock_map_.find(rid) == row_lock_map_.end()) {
            row_lock_map_latch_.unlock();
            txn->SetState(TransactionState::ABORTED);
            throw bustub::TransactionAbortException(txn->GetTransactionId(), AbortReason::ATTEMPTED_UNLOCK_BUT_NO_LOCK_HELD);
        ...
        for (auto lock_request : lock_request_queue->request_queue_) {  // NOLINT
            if (lock_request->txn_id_ == txn->GetTransactionId() && lock_request->granted_) {
                // 找到了先移除，再通过条件变量通知，并解锁lock_request_queue
                lock_request_queue->request_queue_.remove(lock_request);
